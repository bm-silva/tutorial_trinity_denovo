...
library(Biobase)
library(fastcluster)
source("/programs/trinityrnaseq-v2.12.0/Analysis/DifferentialExpression/R/heatmap.3.R")
load("diffExpr.P1e-3_C2.matrix.RData")
data = heatmap_data
gene_partition_assignments <- cutree(as.hclust(hc_genes), k=3)
write.table(gene_partition_assignments[hc_genes$order], file="clusters_fixed_Ktree_3.heatmap.heatmap_gene_order.txt", quote=F, sep='	')
max_cluster_count = max(gene_partition_assignments)
outdir = "diffExpr.P1e-3_C2.matrix.RData.clusters_fixed_Ktree_3"
dir.create(outdir)
...
write.table(gene_colors_dframe, file="clusters_fixed_Ktree_3.heatmap.gene_cluster_colors.dat", quote=F, sep='	')
gene_colors = as.matrix(partition_colors[gene_partition_assignments])
pdf("clusters_fixed_Ktree_3.heatmap.heatmap.pdf")
